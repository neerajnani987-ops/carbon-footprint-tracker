...
const AIAssistant: React.FC = () => {
  ...
  const generateAIResponse = (userText: string): string => {
    ...
    if (text.includes('analyze') || text.includes('monthly') || text.includes('report') || text.includes('footprint')) {
      if (!hasCompletedCalc) {
...
Your highest emission sector is **${highestSector}**. I recommend reviewing your inputs in this category. For example, if you commute via a gasoline vehicle, trying to carpool or walk just 1-2 days a week can significantly lower your overall score.`;
    ...
    if (text.includes('transport') || text.includes('car') || text.includes('flight') || text.includes('travel')) {
...
• **Consider an EV**: If buying a new car, electric vehicles (EVs) have an emissions rate that is 80% lower than petrol vehicles.`;
    ...
    if (text.includes('electric') || text.includes('energy') || text.includes('power') || text.includes('bill')) {
...
• **Eat Local**: Buying local produce cuts food freight transportation emissions.`;
  ...
  return (
    <div className="glass-card border border-white/5 flex flex-col h-[calc(100vh-12rem)] max-w-4xl mx-auto overflow-hidden">
      {/* Header bar */}
      <div className="p-4 bg-eco-forest/20 border-b border-white/5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-eco-green/15 border border-eco-green/20 rounded-xl flex items-center justify-center text-eco-green relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 flex flex-col gap-4">
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              <div className={`w-8.5 h-8.5 rounded-xl border flex items-center justify-center shrink-0 text-xs ${
                isUser
                  ? 'bg-eco-green/20 text-eco-green border-eco-green/20'
              ...
              <div className={`p-4 rounded-2xl text-xs leading-relaxed whitespace-pre-wrap ${
                isUser
                  ...
                  : 'bg-white/3 border border-white/5 text-eco-text rounded-tl-none'
              }`}>
                {msg.text}
              </div>
            </div>
        ...
        {isTyping && (
          <div className="flex gap-3 self-start max-w-[85%]">
            <div className="w-8.5 h-8.5 rounded-xl border bg-white/5 text-white border-white/10 flex items-center justify-center shrink-0 text-xs">
              <Leaf className="w-4 h-4" />
            </div>
            <div className="p-4 bg-white/3 border border-white/5 text-eco-text rounded-2xl rounded-tl-none flex items-center gap-1 w-16 justify-center">
        ...
        <div className="px-6 py-2 flex flex-wrap gap-2 justify-center shrink-0">
          {suggestedQuestions.map((q, idx) => (
            <button
              ...
              className="px-3.5 py-2 bg-white/5 border border-white/10 rounded-full text-[10.5px] font-medium text-eco-muted hover:text-white hover:bg-white/10 transition-all font-outfit cursor-pointer"
            >
              {q}
            </button>
          ))}
      ...
      <div className="p-4 bg-eco-forest/10 border-t border-white/5 shrink-0 flex gap-3 items-center">
        <input
          ...
          className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl focus:border-eco-green/40 focus:bg-white/10 text-white placeholder-eco-muted/50 focus:outline-none transition-all text-xs font-outfit"
          disabled={isTyping}
        />
        <button
